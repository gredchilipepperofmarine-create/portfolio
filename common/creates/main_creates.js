...
.then(data => {
  ...
  creates.innerHTML= data.map(create => `
    <div class="col-12 col-md-6 col-lg-4">
      <div class="card-hover-wrapper h-100">
        <button type="button" class="btn text-start p-0 w-100 h-100 card border-0 shadow-custom card-hover-inner rounded-3" data-bs-toggle="modal" data-bs-target="#modal-${create.title}">
  ...
  createsModal.innerHTML= data.map(create => `
            ...
            <div class="row g-4 align-items-start">
              ...
              <div class="col-12 col-md-7 d-flex flex-column justify-content-between">
                <div>
                  ...
                  <p class="mb-2"><strong>制作概要：</strong>${create.description}<small class="text-muted"><br>${create.note1}</small><br>${create.note2}</small>
                  <p class="mb-2"><strong>工夫した点：</strong><br>
                    <span class="text-danger fw-bold">${create.criativity}</span>
                  </p>
                </div>
                <hr class="my-3">
